...
def get_answers():
    ...
    answer_2 = c.fetchall()
    c.execute("SELECT request_date,round((count_errors*100.0)/total,3) as percentage from total_requests join total_errors on total_requests.request_date = total_errors.error_date  limit 1;")
    answer_3 = c.fetchall()
    db.close()
